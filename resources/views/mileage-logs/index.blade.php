<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('mileage-logs.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                {{ __('New Mileage Log') }}
            </a>
        </div>
    </x-slot>
                                ...
                                @forelse($mileageLogs as $log)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('mileage-logs.show', $log) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                            @if(!$log->is_submitted)
                                                <a href="{{ route('mileage-logs.edit', $log) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
